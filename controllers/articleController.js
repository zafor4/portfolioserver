...
module.exports.createArticle = async (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    console.log("Article Fields:", articleFields);
...
module.exports.toggleArticle = async (req, res) => {
    console.log("Toggle article called");
  const articleId = req.params.id;
  ...
  // flip the highlight boolean
  article.highlight = !article.highlight;

  const updated = await article.save();
  return res.status(200).send({
    message: "Article highlight toggled",
    data: _.pick(updated, ["user", "name", "description", "highlight"]),
  });
};
